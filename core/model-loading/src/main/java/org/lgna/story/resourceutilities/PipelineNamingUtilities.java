...
//        "LEFT_EYELID",
  ...
  private static String[] sArrayNamesToSkip = {"RIGHT_RING", "RIGHT_THUMB", "RIGHT_INDEX", "RIGHT_PINKY", "RIGHT_MIDDLE", "LEFT_RING", "LEFT_THUMB", "LEFT_INDEX", "LEFT_PINKY", "LEFT_MIDDLE", "DOOR", "JAW"};
  ...
  public static String getAliceJointNameForMayaJointName(String mayaName, boolean preserveName) {
    ...
    if (!preserveName) {
      if (sMayaNameToAliceNameMap.containsKey(mayaName)) {
        ...
        System.out.println("No alice name found for " + mayaName + ", autogen name: " + aliceName);
      }
    } else {
      String[] parts = AliceResourceClassUtilities.fullStringSplit(mayaName);
      aliceName = getEnumNameForNameParts(parts);
      System.out.println("Preserving name for " + mayaName + ", autogen name: " + aliceName);
    }
    return aliceName;
  }
