...
- [Save Menu Dialog Write/Readback Proof](./reference/save-menu-dialog-write-proof.md) - Target contract for the `save-menu-dialog-write-proof` QA scenario that runs the rendered File-menu Save, controlled Swing chooser, `.a3p` write, readback, and marker proof path without workflow timeout wiring.
- [Save Proof Evidence](./reference/save-proof-evidence.md) - Target canonical JSON artifact contract, fail-closed validation rules, and executable blocker semantics for the rendered Save proof path.
