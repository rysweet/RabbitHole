...
- [Simple if-statement decode reference](./reference/simple-if-statement-decode.md) - target Tweedle decoder contract for simple `if (condition) { ... }` bodies with supported conditions and an explicit conditional-body allowlist including zero-argument `this.method();` calls.
...
- [Tutorial: Trace simple if-statement decode](./tutorials/simple-if-statement-decode.md) - guided example for asserting the target `ConditionalStatement` shape, allowlisted body statements, and unsupported neighboring conditional-body cases.
