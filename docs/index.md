...
- [Decode coverage characterization](./reference/decode-coverage-characterization.md) - build contract, API behavior, examples, and tutorial guidance for Tweedle, planned literal arithmetic field initializers, player archive, type archive boundaries, and resource decode tests.
...
- [Player archive unsupported Tweedle diagnostics](./reference/player-archive-unsupported-tweedle-diagnostics.md) - narrow JSON `.a3w` archive contract for surfacing unsupported argument-bearing explicit `this` call reasons while keeping planned literal arithmetic field-initializer support scoped.
