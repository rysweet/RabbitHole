...
- [Legacy Program JSON Archive Boundary](./reference/legacy-program-json-archive-boundary.md) - planned fail-closed `JsonProjectIo.readProject` contract for `.a3w` archives whose manifest names `Program`, supported decoding fails, and the narrow one-image recovery path does not apply.
...
- [Characterize the Legacy Program JSON Archive Boundary](./howto/characterize-legacy-program-json-archive-boundary.md) - how to add or review planned negative `IoUtilitiesTest` coverage for `Program` archives after supported decoding fails and one-image recovery does not apply.
...
- [Tutorial: Trace the Legacy Program JSON Archive Boundary](./tutorials/trace-legacy-program-json-archive-boundary.md) - guided walk-through of the planned unsupported `Program` archive failure path and non-claims.
