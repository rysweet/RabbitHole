...
manifest-declared JSON `.a3w` player type. That source no longer belongs to the
unsupported-diagnostics path. The player reader decodes the field initializer as
an AST expression and still rejects broader initializer forms. This shard does
not add `.a3c` type archive support.
...
the arithmetic shard to `.a3c` type archives, or add general Tweedle/player
decode support.
