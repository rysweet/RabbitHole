...
This reference defines the behavior seam for proving that selecting
...
The checked-in `ProcedureTabSelection` surface selects a procedure tab, reports
the selected `UserMethod`, exposes the selected `CodeComposite`, and observes
the selected tab's backing `CodeEditor.getCode()` value. The seam remains
read-only and must not be cited as an edit, Save, rendering, assessment, or full
...
This is a read-only tab/model backing proof. It does not edit the
...
procedure `CodeComposite`. Throws `IllegalStateException` when a selected
procedure tab is not backed by a `CodeEditor`.
