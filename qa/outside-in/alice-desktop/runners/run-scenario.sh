...
validate_allowed_automation() {
  ...
  if [ "$cwd" = . ] &&
    ...
    [ "$7" = netbeans ] &&
    [ "$8" = -am ] &&
    { [ "$9" = -Dtest=org.alice.netbeans.project.ProjectCodeGeneratorStoryApiGeneratedSourceTest#generatedStoryApiSimulationSpeedCallUpdatesRuntimeStateHeadlessly ] || [ "$9" = -Dtest=org.alice.netbeans.project.ProjectCodeGeneratorStoryApiGeneratedSourceTest ]; } &&
    [ "${10}" = test ]; then
    return 0
  fi
